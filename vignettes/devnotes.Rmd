...
Package `dsdb.plus` defines the `SingleCellAccessibilityExperiment`, which is used to store the Tile Matrix of `ArchR` projects. Since this package may not be available to all users, its use here is optional: depending on whether the package is installed or not, the Tile Matrix experiment will be rendered as a `SingleCellAccessibilityExperiment` or a normal `SingleCellExperiment`.
