...
makeMakeMetadata <- function(dataset) {
    ...
    writeLines(con = fileName, text = c(
        ...
        paste0("    BiocVersion = ", as.character(utils::packageVersion("BiocVersion")[,1:2])),
...
makeR <- function(dataset) {
    ...
    writeLines(con = fileName, text = c(
        ...
        "    }",
        "",
        "# place methods to convert SingleCellExperiment objects to custom child classes here",
