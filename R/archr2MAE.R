...
#' @param numThreads Number specifying threads to be used for parallel computing during ArchR project setup
#'
...
archr2MAE <- function(
  ...
  all.exp <- .create.exp.list.from.archr(
    archrDir = archrDir,
  ...
  se.names <- c(
    tile.matrix.names[order(as.numeric(sub('TileMatrix','',tile.matrix.names)))],
...
.create.exp.list.from.archr <- function(
  archrDir,
  numThreads = getArchRThreads(),
) {
  ...
  if(! 'Save-ArchR-Project.rds' %in% list.files(archrDir)) {
    stop('Are you sure this is an ArchR project directory? No Save-ArchR-Project.rds file found in the ArchR project directory');
  }
...
.create.embedding.map <- function(archrProj) {
  ...
  reduced.dim.matrix.type <- unlist(sapply(reduced.dim.names,function(x) {archrProj@reducedDims[[x]]$useMatrix}));
  
  embedding.colnames <- sapply(names(archrProj@embeddings),function(x) {colnames(getEmbedding(archrProj, x))});
...
.assign.tile.rowranges <- function(se, chrSizes) {
  ...
  if(length(tile.size) == 1) {
    ...
    for(chr in seqlevels(rowRanges(se))){
      ...
      end(rowRanges(se))[as.logical(seqnames(rowRanges(se)) == chr) & end(rowRanges(se)) > chr.end] <- chr.end;
    }
...
.load.matrix <- function(matrix.type, archrProj, embedding.map = NULL, numThreads = getArchRThreads()) {
  ...
  if('projColData[, colnames(projColData) %ni% colnames(colData)]' %in% colnames(colData(se))) {
    ...
    if(length(missing.colname) == 1 && length(which(colnames(colData(se)) == 'projColData[, colnames(projColData) %ni% colnames(colData)]')) == 1) {
      colnames(colData(se))[which(colnames(colData(se)) == 'projColData[, colnames(projColData) %ni% colnames(colData)]')] <- missing.colname;
    }
  }
